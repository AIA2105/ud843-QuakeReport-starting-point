...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray earthquickArray = root.getJSONArray("features");

            SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM dd , yyyy\nh:mm a");

            for (int i=0 ; i<earthquickArray.length() ; i++) {
                JSONObject current = earthquickArray.getJSONObject(i);
                JSONObject propeties = current.getJSONObject("properties") ;
                        ...
                        String date = dateFormatter.format(new Date(Long.valueOf(time)));

                        String mag = new DecimalFormat("0.0").format(Double.valueOf(magnitude));
                        String parts[] = location.split("(?<= of )");
                        String p [] = new String[2]; int k = 0;
                        for (String a : parts){p[k]=a; k++;}
                    ...
                    earthquakes.add(new Earthquake(mag,p[0],p[1],date,url));
            }
